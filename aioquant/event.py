...
class EventStatus(Event):
    ...
    def __init__(self, status: Status):
        ...
        name = "EVENT_STATUS"
        exchange = "Status"
        routing_key = "{p}.{s}".format(p=status.platform, s="Status")
        queue = "{sid}.{ex}.{rk}".format(sid=config.server_id, ex=exchange, rk=routing_key)
        super(EventStatus, self).__init__(name, exchange, queue, routing_key, data=status.smart)

    def parse(self):
        tatus = Status().load_smart(self.data)
        return tatus
    

class EventConfigSet(Event):
    
    def __init__(self, configset: ConfigSet):
        """Initialize."""
        name = "EVENT_CONFIG_SET"
        exchange = "ConfigSet"
        routing_key = "{p}.{s}".format(p=configset.platform, s=configset.strategy)
        queue = "{sid}.{ex}.{rk}".format(sid=config.server_id, ex=exchange, rk=routing_key)
        super(EventConfigSet, self).__init__(name, exchange, queue, routing_key, data=configset.smart)
        
    def parse(self):
        set_msg = ConfigSet().load_smart(self.data)
        return set_msg
...
class EventCenter:
    ...
    async def connect(self, reconnect=False):
        ...
        exchanges = ["Orderbook", "Kline", "Trade", "Status", "ConfigSet"]
    ...
    async def _initialize(self, event: Event, callback=None, multi=False):
        ...
        if callback:
            if multi:
                ...
                logger.debug("multi message queue:", queue_name, caller=self)
            else:
                await self._channel.basic_consume(self._on_consume_event_msg, queue_name=queue_name)
                logger.debug("queue:", queue_name, caller=self)
